# -------------------------------------------------------------------------------
# Naam:		        vliegtuigen_rondom_locatie
...
#	    	        een opgegeven locatie vallen, op te slaan in een database
#
...
def start():
    ...
    print("start")
    try:
        conn = sqlite3.connect("./data/vliegtuigen.db")
        cur = conn.cursor()
        listOfTables = cur.execute("""SELECT tbl_name FROM sqlite_master WHERE type='table' AND tbl_name in('hoge_vliegtuigen','lage_vliegtuigen','meetpunten');""").fetchall()
        ...
        print(e)
        print("Geen SQLite database gevonden")
        return False
